                ...
                <div class="caption">
                    ...
                    <div class="row">
                        <div class="col-xs-12 col-md-6">
                            <a class="btn btn-danger btn-flat" onClick="deleteConfirmation('<?php echo $row->SERVICE_ID ?>','<?php echo $row->SERVICE_NAME?>','<?php echo $row->IMG_PATH?>')">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
...
function deleteConfirmation(servId, servName, serv_pic) {
    $(".modal-body").text("Are you sure want to delete " + servName + " Service");
    $("#modal-danger").modal({
        show: true
    });
    serviceId = servId;
    service_pic = serv_pic;
...
function deleteService() {
    var request = $.ajax({
        url: "<?php echo base_url(); ?>index.php?/admin/services/removeServices/" + serviceId + '/' + service_pic,
        type: "POST"
    });

    request.done(function(msg) {
        $("#modal-danger").modal('hide');
        $("#modal-success").find( ".modal-body" ).text("The " + service_name + " service deleted succesfully");
        $("#modal-success").modal({
            show: true
        });
    ...
    $("#modal-danger").modal('hide');
        $("#modal-warning").find( ".modal-body" ).text("Unable to delete" + service_name);
        $("#modal-warning").modal({
            show: true
        });
    });
